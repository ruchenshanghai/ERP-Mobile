  ...
  <div>
    ...
    <template v-for="order in displayList">
      <group :title="'单据编号: ' + order.billNo + ' 业务类别: ' + order.transTypeName">
        <router-link :to="{'name': 'SalesOutboundDetail', 'params': {'ID': order.id}}">
          <grid :cols="3" class="grid-order">
            ...
            <grid-item label="销售数量">
              {{order.saleQty}}
            </grid-item>
            <grid-item label="出库数量">
              {{order.totalQty}}
            </grid-item>
            <grid-item label="出库状态">
              {{order.billStatus}}
            </grid-item>
            <grid-item label="制单人">
              {{order.userName}}
  ...
  export default {
    ...
    methods: {
      fetchOrderListByConfig () {
        ...
        this.$http.post(this.config.Warehouse.SalesOutbound.fetchURL, postData).then(orderRes => {
